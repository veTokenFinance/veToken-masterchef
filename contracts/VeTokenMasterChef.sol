 ...
 * @author  ConvexFinance / AuraFinance modified for VeTokenFinance by st4rgard3n - Raid Guild
 ...
 */
contract VeTokenMasterChef is Ownable, ReentrancyGuard {
    ...
    struct UserInfo {
        ...
        //
        //   pending reward = (user.amount * pool.acCvxPerShare) - user.rewardDebt
        //
        // Whenever a user deposits or withdraws LP tokens to a pool. Here's what happens:
        //   1. The pool's `acCvxPerShare` (and `lastRewardBlock`) gets updated.
    ...
    struct PoolInfo {
        ...
        uint256 acCvxPerShare; // Accumulated CVXs per share, times 1e12. See below.
        IRewarder rewarder;
    }
    ...
    // Bonus multiplier for early cvx makers.
        ...
        returns (uint256)
    {
        ...
        uint256 acCvxPerShare = pool.acCvxPerShare;
        ...
        if (block.number > pool.lastRewardBlock && lpSupply != 0) {
            ...
            uint256 cvxReward = multiplier
                ...
                .mul(pool.allocPoint)
                .div(totalAllocPoint);
            acCvxPerShare = acCvxPerShare.add(
                cvxReward.mul(1e12).div(lpSupply)
            );
        }
        return user.amount.mul(acCvxPerShare).div(1e12).sub(user.rewardDebt);
    }
    ...
    function updatePool(uint256 _pid) public {
        ...
        pool.acCvxPerShare = pool.acCvxPerShare.add(
            cvxReward.mul(1e12).div(lpSupply)
        );
        pool.lastRewardBlock = block.number;
    }
    ...
    function deposit(uint256 _pid, uint256 _amount) public nonReentrant {
        ...
        user.rewardDebt = user.amount.mul(pool.acCvxPerShare).div(1e12);
    ...
    function withdraw(uint256 _pid, uint256 _amount) public nonReentrant {
        ...
        updatePool(_pid);
        uint256 pending = user.amount.mul(pool.acCvxPerShare).div(1e12).sub(
            user.rewardDebt
        );
        safeRewardTransfer(msg.sender, pending);
